    ...
    <div>
        ...
        <swiper :options="swiperOption">
            <swiper-slide v-for=" v in tags" :key="v.id">
                <router-link :to="{params:{tid:v.id},name:'Video'}">{{v.name}}</router-link>
            </swiper-slide>
        </swiper>
        <!--导航条结束-->
...
<script>
    export default {
        ...
        methods: {
            loadData() {
                //获取标签
                this.axios.get('http://web.blog.com:8080/api/tags').then((response) => {
                    this.tags = response.data.data;
                })
                //获取课程
                let tid = this.$route.params.tid;
                this.axios.get('http://web.blog.com:8080/api/lesson/' + (tid ? tid : 0)).then((response) => {
                    if (response.data.code != 200) {
                        alert('获取课程列表失败，请重新获取');
                    } else {
